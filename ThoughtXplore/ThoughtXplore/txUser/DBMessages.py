...
def decode(result,rescode,fun):
    if result == 1:
        if fun == 'InsertUserFromSite':
            ...
            return 'User already exists. An email has been sent to registration email address. Please follow the instructions in the email for verification. In case, there is no email, please check spam folder and add "no-reply@thoughtexplore.com" as a valid email adress.in case of no-email, feel free to contact system administrators'
    else: 
